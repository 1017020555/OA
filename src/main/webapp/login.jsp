...
<script type="text/javascript">
	$(function () {
    	$("#code").click(function () {
			$(this).prop("src","${pageContext.request.contextPath}/code/getCode?time="+new Date());
        });
    });
	function login() {
		$.ajax({
			method:"get",
			url:"${pageContext.request.contextPath}/code/login",
			data:{"username":$("#username").val(),"password":$("#password").val(),"path":$("#path").val()}
    });
    }
</script>
	<DIV>
											...
											<TBODY>
												...
												<TR>
													...
													<TD><INPUT class=regtxt title=请填写密码 type=password
														maxLength=16 size=16 name="password" id="password"></TD>
												</TR>
												<TR>
													<TD align=middle width=81><FONT color=#ffffff>验证码：</FONT></TD>
													<TD>
														<INPUT class=regtxt title=请填写验证码 maxLength=50
														size=16 id="path" name="path">
														<img name="code" id="code" src="${pageContext.request.contextPath}/code/getCode?time='+new Date()'">
												</TR>
											</TBODY>
										</TABLE>
									</TD>
